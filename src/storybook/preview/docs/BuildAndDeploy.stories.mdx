...
## To prepare the component for deployment
...
```
and add ``"@stryberventures/gaia-react.button": ">=0.1.0"`` the ``Button`` source will not be
included to component's build

And vice versa, if the ``@stryberventures/gaia-react.button`` is not in the peer dependency list,
