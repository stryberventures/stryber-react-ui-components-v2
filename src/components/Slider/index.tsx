...
export const Slider = (props: ISlider) => {
  ...
  const [minVal, setMinVal] = useState(disabled ? minValue : (Array.isArray(fieldValue) ? fieldValue[0] : fieldValue || minValue));
  const [maxVal, setMaxVal] = useState(disabled ? maxValue : (fieldValue ? fieldValue[1] : maxValue));
