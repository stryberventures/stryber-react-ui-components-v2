...
const SnackbarContent = React.forwardRef<HTMLDivElement, ISnackbarContentProps>(
  (props, ref) => {
    const {
      variant,
      message,
    ...
    return (
      <div
        ref={ref}
        className={classNames(classes.snackbarContent, className)}
        style={style}
      >
        <div className={classes.mainContent}>
          {variant !== 'default' && withIcon && (
            <div className={classes.symbol}>{renderIcon()}</div>
          )}
          <div className={classes.message}>
            ...
            {description && (
              <Text variant="components2" className={classes.description}>
                {description}
              </Text>
            )}
          </div>
        ...
        {variant !== 'default' && !!onClose && (
          <div onClick={onClose} data-testid="gaia-snackbar-close-button">
            <Icons.CloseIcon
              fill={theme.colors[variantToColor()!]?.dark600}
