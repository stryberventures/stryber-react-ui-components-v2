...
const CheckboxGroup: React.FC<ICheckboxGroupProps> = (props) => {
  const { name = '', checkboxes, disabled, errorMessage, ...rest } = props;
  ...
  const handleParentCheckboxChange = () => {
    ...
    onChangeWrapper(
      setAllChildVal(!checkChildValues(enabledCheckboxes, 'some'))
    );
  };
  ...
  return (
    <div className={classNames(classes.checkboxGroup, rest.className)}>
      ...
      <div className={classes.childCheckboxes}>
        {Object.values(childCheckboxes).map((childProps: TChildCheckbox) => {
          return (
            <Checkbox
              controlled
              ...
              onChange={(e: BaseSyntheticEvent) =>
                handleChildCheckboxChange(e, childProps.name)
              }
              key={childProps.name}
              color={rest.color}
