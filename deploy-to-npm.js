...
const getNpmVersion = (packageName) => {
  return new Promise((resolve, reject) => {
    shell.exec(`npm view ${packageName} version`, { silent: true }, (code, stdout, stderr) => {
      if (code === 0) {
        resolve(stdout);
      } else {
...
const getPackagesToDeploy = async () => {
  ...
  console.log('');
  console.log("\x1b[36m", 'Comparing built packages versions with npm versions...');
  console.log("\x1b[0m");
...
const deployPackages = (packages) => {
  if (packages.length) {
    console.log('');
    console.log('Packages to deploy', packages.map(({ name, version }) => `${name}@${version}`));
      ...
      try {
        if (fs.existsSync(`${buildDir}/${dir}`)) {
          ...
          shell.exec(`npm publish --access public`, (code, out, err) => {
            ...
            console.log("\x1b[32m", `${name}@${version} successfully published`);
          });
          shell.cd('../../');
        } else {
